...
public static class GitPushFailureClassifier
{
    ...
    public static string ToUserMessage(string output, GitHostingProvider? provider = null) =>
        LooksLikeMissingPushPermission(output)
            ? $"{(provider?.DisplayName() ?? "Git")} 平台拒絕推送。請確認目前 Windows Git 憑證具有此 repository 的寫入權限，或重新登入 Git Credential Manager。"
