...
.\scripts\publish.ps1
# 或指定版本
.\scripts\publish.ps1 -Version 1.5.0
# 只要單一 exe、不要安裝程式
...
vpk pack --packId Hugoer --packVersion 1.5.0 --packDir .\dist\publish\win-x64 --mainExe Hugoer.exe --outputDir .\dist\releases\velopack
```
...
7. **GitHub** → `gh auth login` → 建立 Repo + 推送 + 啟用 Pages  
8. **GitHub** → 查看「線上版本監控」；Hugoer 每 5 分鐘確認 Pages 是否已更新至本次推送版本
