...
The UI uses a deterministic `DemoStore` so the screens can be reviewed without credentials. Replace that adapter with the authenticated GitHub/desktop bridge when wiring production data. The app deliberately keeps local Hugo installation, bulk migration, and local server control as desktop hand-off tasks.
